...
if (isset($_SESSION['user'])) {
    ...
    if (isset($_POST['ok'])) {
        ...
        $total_ventes = htmlspecialchars($_POST['total_ventes']);
        $date_vente = $_POST['date'];


        // Vérifier que les données nécessaires sont présentes
        if ($id_client && !empty($produit_ids) && !empty($quantites) && !empty($total) && !empty($date_vente) && !empty($total_ventes)) {
            $bdd->beginTransaction(); // Début de la transaction

           // var_dump($id_client, $produit_ids, $quantites, $total, $date_vente, $total_ventes);
            try {
                $id_vente = [];
                ...
                foreach ($produit_ids as $index => $id_produit) {
                    ...
                    $stmt = $bdd->prepare("INSERT INTO vente (id_client, id_produit, quantite, prix_vente, total, date_vente, total_ventes) VALUES (:id_client, :id_produit, :quantite, :prix_vente, :total, :date_vente, :total_ventes)");
...
// Récupérer total_achats pour la période donnée
$stmt_achats = $bdd->prepare("SELECT SUM(total_achats) as total_achats FROM achat WHERE date_achat = :date_vente");
$stmt_achats->bindParam(':date_vente', $date_vente);
$stmt_achats->execute();
$result_achats = $stmt_achats->fetch(PDO::FETCH_ASSOC);
if ($result_achats) {
    $total_achats = $result_achats['total_achats'];
}

// Calculer la recette comme la différence entre total_ventes et total_achats
$revenue = $total_ventes - $total_achats;

$stmt_recette = $bdd->prepare("
    INSERT INTO recette (period, period_type, total_achats, total_ventes, revenue) 
    VALUES (:date_vente, 'daily', :total_achats, :total_ventes, :revenue) 
    ON DUPLICATE KEY UPDATE 
        total_ventes = total_ventes + :total_ventes,
        revenue = revenue + :revenue
");
$stmt_recette->bindParam(':date_vente', $date_vente);
$stmt_recette->bindParam(':total_achats', $total_achats);
$stmt_recette->bindParam(':total_ventes', $total_ventes);
$stmt_recette->bindParam(':revenue', $revenue);
...
$bdd->commit();
// if ($stmt->execute()) {
//     $id_vente = $bdd->lastInsertId(); // Récupérer l'ID de la vente ajoutée
//     header("Location: facture_one.php?id_vente=" . $id_vente);
//     exit;
// }
                $erreurs = "Vente bien ajouté !";
...
<main class="content px-3 py-2">
                ...
                <form action="" method="post">
                    ...
                    <div id="productEntries">
                        <div class="productEntry mb-3 row">
                            ...
                            <div class="col-sm-10">
                                <select name="produit_id[]" class="form-select productSelect" aria-label="Default select example">
                                    ...
                                    <?php foreach ($produits as $produit) { ?>
                                        <option value="<?= $produit['id_produit'] ?>" data-price="<?= $produit['prix_vente'] ?>"><?= htmlspecialchars($produit['nom_produit']) ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            ...
                            <div class="col-sm-10">
                                <input type="number" name="prix_vente[]" step="0.01" class="form-control prix" placeholder="Prix vente" readonly>
                            </div>
                           <!--  <label for="prix" class="col-sm-2 col-form-label">Prix de vente</label>
                            <div class="col-sm-10">
                                <input type="number" name="prix_vente[]" step="0.01" class="form-control" placeholder="Prix de vente" readonly>
                            </div> -->
                    ...
                    <div class="mb-3 row">
                        <label for="total_ventes" class="col-sm-2 col-form-label">Total achats</label>
                        <div class="col-sm-10">
                            <input type="text" name="total_ventes" class="form-control" id="total_ventes" placeholder="Total ventes" readonly>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="date" class="col-sm-2 col-form-label">Date</label>
                        <div class="col-sm-10">
                            <input type="date" name="date" class="form-control" id="date_vente">
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <div class="col-sm-10 offset-sm-2">
  ...
  <!--   <a href="facture_one.php?id_facture=<?= $id_facture ?>" class="btn btn-outline-success mb-3"> -->                         <a href="vente.php" class="btn btn-outline-warning mb-3">Annuler</a>
                        </div>
                    </div>
                </form>
            </div>
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    function calculateTotalSale() {
        ...
        document.getElementById('total_ventes').value = total_ventes.toFixed(2);
    }
});
</script>
