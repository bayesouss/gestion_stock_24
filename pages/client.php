...
$sql = "SELECT COUNT(*) AS nbclient FROM client WHERE nom LIKE :cherche OR prenom LIKE :cherche OR telephone LIKE :cherche";
$result = $bdd->prepare($sql);
$result->bindValue(':cherche', '%' . $cherche . '%', PDO::PARAM_STR);
$result->execute();
$nbTotalclient = $result->fetch();
$total = (int)$nbTotalclient['nbclient'];
...
$sql = "SELECT * FROM client WHERE nom LIKE :cherche OR prenom LIKE :cherche OR telephone LIKE :cherche ORDER BY id_client LIMIT :premier, :parpage";
...
$result1->execute();
$clients = $result1->fetchAll(PDO::FETCH_ASSOC);

$total_clients = $bdd->query("SELECT COUNT(*) as total_sales FROM client")->fetch(PDO::FETCH_ASSOC)['total_sales'];
        ...
        <div class="card-body py-4">
            <div class="d-flex align-items-start">
                <div class="flex-grow-1">
                    ...
                    <h4 class="mb-2">
                        <?php echo htmlspecialchars($total_clients); ?> Clients
                    </h4>
            ...
            <div class="card-body">
                <form method="get" action="client.php">
                    <div class="row">
                        ...
                        <div class="col-auto">
                            <a href="formclient"><i class="fas fa-plus"></i>Nouveau Client</a>
                        </div>
                    </div>
                </form>
            </div>
                    ...
                    <tbody>
                        <?php foreach($clients as $client) { ?>
                            <tr>
                                ...
                                <td><?= $client['email'] ?></td>
                                <td>
                                    <a href="modifclient.php?idC=<?php echo $client['id_client'] ?>"><i class="fas fa-edit"></i></a>
                                    &nbsp;
                                    &nbsp;
                                    <a onclick="return confirm('Etes-vous sûr de vouloir supprimer le client?')" href="supprimclient.php?idCl=<?php echo $client['id_client'] ?>"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
                    ...
                    <div class="col-auto">        
                        <ul class="pagination">
                            <li class="page-item <?= ($currentpage == 1) ? "disabled" : "" ?>">
                                <a href="client.php?cherche=<?= urlencode($cherche) ?>&page=<?= $currentpage - 1 ?>" class="page-link">Precedent</a>
                            </li>
                            <?php for($page = 1; $page <= $pages; $page++): ?>
                                <li class="page-item <?= ($currentpage == $page) ? "active" : "" ?>">
                                    <a href="client.php?cherche=<?= urlencode($cherche) ?>&page=<?= $page ?>" class="page-link"><?= $page ?></a>
                                </li>
                            <?php endfor ?>
                            <li class="page-item <?= ($currentpage == $pages) ? "disabled" : "" ?>">
                                <a href="client.php?cherche=<?= urlencode($cherche) ?>&page=<?= $currentpage + 1 ?>" class="page-link">Suivant</a>
                            </li>
                        </ul>
                    </div>
                </div>
